...
- For this repository, run test and typecheck commands only through the user's external `pnpm` command. Do not run `vitest`, `tsc`, or their entrypoints via direct `node` invocation for routine work.
...
- In this repo's Codex environment, `apply_patch` has proven unreliable in practice. Prefer a minimal direct file rewrite after verifying contents instead of repeatedly retrying `apply_patch`.
...
- Tests and type checks in this repo must be run via external-console pnpm (pnpm test, pnpm typecheck, or another pnpm-wrapped project command). Treat direct 
ode execution of Vitest/TypeScript as a last-resort diagnostic exception, not a normal workflow.
