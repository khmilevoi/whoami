...
const execute = async (ctx: Context, logger: LoggerPort, action: () => Promise<void>): Promise<void> => {
  try {
    await action();
  } catch (error) {
...
export const registerTelegramHandlers = (bot: Bot, gameService: GameService, logger: LoggerPort): void => {
  bot.command("start", async (ctx) => {
    await execute(ctx, logger, async () => {
      if (isPrivate(ctx)) {
