...
describe("normal mode service", () => {
  it("sends pre-start disclosure to every player", async () => {
    ...
    const started = await components.game.setupNormalOnlineRandomInProgress(chatId, actors);
    ...
    const disclosureMessages = components.game.notifier.sent.filter(
      (entry) => entry.kind === "private-message" && entry.text.startsWith("Список слов других игроков:"),
    );
  ...
  it("handles online question flow and resolves votes", async () => {
    ...
    await components.normalMode.handleGroupText(chatId, asker.telegramUserId, "Question 1");
    ...
    expect(
      components.game.notifier.sent.some(
        (entry) => entry.kind === "group-keyboard" && entry.chatId === chatId && entry.text.includes("Голосуем"),
      ),
    ).toBe(true);
