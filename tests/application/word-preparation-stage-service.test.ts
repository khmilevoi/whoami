...
describe("word preparation stage service", () => {
  it("reuses the same word/clue/finalization flow for reverse mode", async () => {
    ...
    await components.configurationStage.applyConfigStep(game.id, actors[0].telegramUserId, "mode", "REVERSE");
    await components.configurationStage.applyConfigStep(game.id, actors[0].telegramUserId, "play", "OFFLINE");

    await components.wordPreparationStage.handlePrivateText(actors[0].telegramUserId, "planet");

    const confirmPrompt = components.game.notifier.sent
      .filter((entry): entry is SentPrivateKeyboard => entry.kind === "private-keyboard" && entry.userId === actors[0].telegramUserId)
      .at(-1);
    expect(confirmPrompt?.text).toContain('Подтвердите слово: "planet"');

    await components.wordPreparationStage.handleWordCallback(game.id, actors[0].telegramUserId, "confirm", "NO");

    expect(
      components.game.notifier.sent.some(
        (entry) => entry.kind === "private-message" && entry.userId === actors[0].telegramUserId && entry.text === "Введите слово заново:",
      ),
    ).toBe(true);

    await components.wordPreparationStage.handlePrivateText(actors[0].telegramUserId, "mars");
    await components.wordPreparationStage.handleWordCallback(game.id, actors[0].telegramUserId, "confirm", "YES");
    await components.wordPreparationStage.handleWordCallback(game.id, actors[0].telegramUserId, "clue", "YES");

    expect(
      components.game.notifier.sent.some(
        (entry) => entry.kind === "private-message" && entry.userId === actors[0].telegramUserId && entry.text === "Введите пояснение:",
      ),
    ).toBe(true);

    await components.wordPreparationStage.handlePrivateText(actors[0].telegramUserId, "red world");
    ...
    await components.wordPreparationStage.handleWordCallback(game.id, actors[0].telegramUserId, "final", "NO");
  ...
  it("starts the game once all players finalize their words", async () => {
    ...
    for (const actor of actors) {
      await components.wordPreparationStage.handlePrivateText(actor.telegramUserId, `word-${actor.telegramUserId}`);
      await components.wordPreparationStage.handleWordCallback(game.id, actor.telegramUserId, "confirm", "YES");
      await components.wordPreparationStage.handleWordCallback(game.id, actor.telegramUserId, "clue", "NO");
      await components.wordPreparationStage.handleWordCallback(game.id, actor.telegramUserId, "final", "YES");
    }
    ...
    const readyMessages = components.game.notifier.sent.filter(
      (entry): entry is SentPrivateMessage =>
        entry.kind === "private-message" && entry.userId === actors[0].telegramUserId && entry.text === "Готово. Ожидаем остальных игроков.",
    );
